...
const MAX_RUN_NUMBER_RETRY_ATTEMPTS = 5;
...
async function resolveAssumptionVersion(): Promise<{
  ...
  try {
    ...
    const seededByAnotherRequest = await db.query.assumptionVersion.findFirst({
      where: and(
        eq(assumptionVersion.versionLabel, currentDef.versionLabel),
        eq(assumptionVersion.productLine, currentDef.productLine),
      ),
    ...
    if (!seededByAnotherRequest) {
      throw error;
    }

    cachedAssumptionVersionId = seededByAnotherRequest.id;
    cachedAssumptionVersionLabel = seededByAnotherRequest.versionLabel;
    return {
      id: seededByAnotherRequest.id,
      versionLabel: seededByAnotherRequest.versionLabel,
    };
  }
}
...
export async function computeGuestEstimate(
  payload: Pick<
    RunEstimateInput,
    | "annualIncome"
    | "age"
  ...
  const needsResult = calculateInsuranceNeedsRounded({
    clientIncome: payload.annualIncome,
    spouseIncome: 0,
  ...
  const recommendedCoverage =
    payload.coverageAmountOverride > 0
      ? payload.coverageAmountOverride
      : needsResult.totalInsuranceNeeds;

  const premiumRange = await mockTermLifeProvider.estimatePremiumRange({
    age: payload.age,
...
export async function runEstimate(
  ...
  // and persist new runs with retry for run_number uniqueness races.
  const db = getDb();
  const maxAttempts = input.clientId ? MAX_RUN_NUMBER_RETRY_ATTEMPTS : 1;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (input.clientId && attempt < maxAttempts && isUniqueViolation(error)) {
        continue;
      }
