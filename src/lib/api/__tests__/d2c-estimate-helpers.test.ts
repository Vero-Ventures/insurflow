...
const {
  ...
  return {
    ...
    mockEstimateRunFindMany: estimateRunFindMany,
    mockInsert: insert,
    mockValues: values,
...
vi.mock("@/server/db", () => ({
  getDb: vi.fn(() => ({
    query: {
      ...
      estimateRun: {
        findFirst: mockEstimateRunFindFirst,
        findMany: mockEstimateRunFindMany,
      },
    },
    insert: mockInsert,
  })),
...
describe("runEstimate", () => {
  ...
  it("retries insert with incremented runNumber after unique conflict", async () => {
    const currentInput = createEstimateInput({ termYears: 20 });
    const latestRun = createPersistedRun(
      createEstimateInput({ termYears: 15 }),
      {
        runNumber: 1,
      },
    );
    ...
    mockEstimateRunFindFirst
      ...
      .mockResolvedValueOnce({ ...latestRun, runNumber: 2 });

    const uniqueViolation = Object.assign(new Error("unique_violation"), {
      code: "23505",
    });
    mockReturning.mockRejectedValueOnce(uniqueViolation).mockResolvedValueOnce([
      {
    ...
    expect(mockValues.mock.calls[0]?.[0]).toMatchObject({ runNumber: 2 });
    expect(mockValues.mock.calls[1]?.[0]).toMatchObject({ runNumber: 3 });
  });

  it("recovers assumption version seeding when concurrent insert wins", async () => {
    const input = createEstimateInput();
    const firstInsertUniqueViolation = Object.assign(
    ...
    mockReturning
      .mockRejectedValueOnce(firstInsertUniqueViolation)
      .mockResolvedValueOnce([
        {
